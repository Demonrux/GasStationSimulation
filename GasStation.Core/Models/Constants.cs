namespace GasStation.Core.Models
{
    public static class Constants   
    {
        public const double TimingСoefficient = 0.7;
        public const double MaxGenerationFactor = 1.3;
        public const double FuelCoefficient = 0.1;
        public const double BaseRefuelingTime = 3.0;
        public const double BasePaymentTime = 2.0;
        public const int MinCarFuelAmount = 10;
        public const int MaxCarFuelAmount = 40;
        public const int FuelTruckAmount = 400;
        public const int FuelThreshold = 100;
    }
}
